...
int main(void)
{
  ...
  /* USER CODE BEGIN Init */
	...
	interrupt_config.latch_int_en = false;
	...
	while (1)
	{
		if (mpu_data_ready) {
			...
			if(MPU6050_ReadAllData(&mpu6050) == HAL_OK) {
				...
				printf("gyro x: %f, gyro y: %f, gyro z: %f, filtered_gyro x: %f, filtered_gyro y: %f, filtered_gyro z: %f\n",
						mpu6050.gyro_angel[0], mpu6050.gyro_angel[1], mpu6050.gyro_angel[2], mpu6050.gyro_angel_f[0], mpu6050.gyro_angel_f[1], mpu6050.gyro_angel_f[2]);
				HAL_Delay(20);
			}
		}
    /* USER CODE END WHILE */
